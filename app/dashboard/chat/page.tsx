  ...
  return (
    <div className="flex flex-col h-[calc(100vh-73px)]">
      ...
      <div className="px-6 py-4 border-b border-white/5 flex items-center gap-4 bg-[#050508]/50 backdrop-blur-xl">
        <div className="w-12 h-12 bg-theme-primary rounded-2xl flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.4)]">
        ...
        <div className="ml-auto flex items-center gap-2 bg-theme-primary/10 border border-theme-primary/20 px-3 py-1.5 rounded-full">
          <Zap size={12} className="text-theme-primary" />
          <span className="text-[9px] font-black text-theme-primary uppercase">2/3 Sesi Gratis</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              {msg.from === "mira" && (
                <div className="w-8 h-8 bg-theme-primary rounded-xl flex items-center justify-center mr-3 shrink-0 mt-1">
      ...
      <div className="px-6 pb-3 flex gap-2 flex-wrap">
        {quickReplies.map((qr) => (
          <button key={qr} onClick={() => send(qr)}
            className="px-3 py-2 bg-white/5 border border-white/10 hover:bg-theme-primary/20 hover:border-theme-primary/30 text-slate-400 hover:text-theme-primary text-[10px] font-bold rounded-xl transition-all"
          >
            {qr}
          </button>
        ))}
      ...
      <div className="px-6 pb-6">
        <div className="flex gap-3 bg-white/5 border border-white/10 rounded-2xl p-2">
          ...
          <button onClick={() => send(input)}
            className="p-3 bg-theme-primary hover:bg-theme-primary text-white rounded-xl transition-all"
          >
            <Send size={16} />
          </button>
        </div>
